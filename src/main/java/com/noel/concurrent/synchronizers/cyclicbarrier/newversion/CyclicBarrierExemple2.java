...
public class CyclicBarrierExemple2 implements Callable<Integer> {

  int start, end, resultat;
  CyclicBarrier barrier;
  String name;

  public CyclicBarrierExemple2(int pStart, int pEnd, String pName) {
    start = pStart;
    end = pEnd;
    name = pName;
  }
  ...
  public Integer call() {
    ...
    //s'attendent lorsque ceux-ci atteignent la moitié de leur travail attitré
    ...
    while (start < end) {
      ...
      if (start == moitie) {
        try {
          System.err.println("Le thread " + name + " a atteint la moitié de sa tâche, start=" + start);
